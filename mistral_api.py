...
def agent_generate_image(prompt: str, size="1024x1024") -> bytes:
    ...
    # 1) Créer l'agent (chat pilotant le connecteur image)
    agent_payload = {
    ...
    r = requests.post("https://api.mistral.ai/v1/agents", json=agent_payload, headers=headers_json, timeout=60)
    ...
    file_resp = requests.get(f"https://api.mistral.ai/v1/files/{file_id}/content", headers=file_headers, timeout=120)
...
def describe_landmark_from_np(np_bgr) -> str:
    ...
    system = (
        "Décris en français le sujet principal. "
        "Si c’est un monument/bâtiment/ouvrage ET que tu es sûr à environ 80% ou plus, "
        "donne son nom officiel, la ville/pays, le style/période, les dates de constructions et quelques éléments distinctifs. "
        "Sinon, ne cite aucun nom et décris seulement la scène en indiquant clairement l’incertitude. "
        "Reste concis (3–5 phrases), texte continu, pas de listes."
    )
    user_content = [
